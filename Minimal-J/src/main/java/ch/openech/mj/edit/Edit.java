...
/**
 * Kind of a light weight editor. Saves synchron and cannot have
 * a followLink
 * 
 * @author bruno
 *
...
public abstract class Edit<T> {
	...
	public IForm<T> startEditor() {
		...
		form = createForm(new EditorChangeListener());
	...
	protected T newInstance() {
		...
		Class<T> clazz = (Class<T>) ch.openech.mj.util.GenericUtils.getGenericClass(Edit.this.getClass());
	...
	private class EditorChangeListener implements IForm.FormChangeListener<T> {
		...
		public void validate(T object, List<ValidationMessage> validationResult) {
			Edit.this.validate(object, validationResult);
		}
